console.log('BabyFood base estable PC cargada v14');
'use strict';
...
function choiceScore(f,counts={},recent={},opts={}){f=enrichFood(f); let score=0; score+=(counts[f.name]||0)*10; score+=(recent[f.name]||0)*18; if(opts.avoidNames&&opts.avoidNames.has(f.name))score+=80; if(opts.preferIron&&f.iron)score-=6; if(opts.needsHydrating&&f.hydrating)score-=10; if(opts.avoidDense&&f.dense)score+=35; if(opts.sameCatPenalty)score+=opts.sameCatPenalty; return score;}
...
function buildLunchFromSafe(existing=[], extraFood=null){const usedSafe=new Set(); const counts=usageCounts(existing); const recent=recentUseMap(existing,4); const avoidNames=lastBlockFoodNames(existing); let lunch=[]; const veggies=getSafeByCat('verdura'); const proteins=getSafeByCat('proteina'); const firstV=pickBalanced(veggies,usedSafe,counts,recent,{avoidNames,preferIron:true,needsHydrating:!!extraFood?.dense}); if(firstV){lunch.push(firstV);usedSafe.add(firstV.name);} const hasDense=()=>dedupe([...lunch,...(extraFood?[extraFood]:[])]).some(f=>f.dense); const secondV=pickBalanced(veggies,usedSafe,counts,recent,{avoidNames,needsHydrating:hasDense(),avoidDense:hasDense()}); if(secondV){lunch.push(secondV);usedSafe.add(secondV.name);} const protein=pickBalanced(proteins,usedSafe,counts,recent,{avoidNames,preferIron:true,avoidDense:hasDense()}); if(protein){lunch.push(protein);usedSafe.add(protein.name);} while(lunch.length<3){const fill=pickBalanced(S.safeFoods.filter(f=>f.cat!=='fruta'&&passesPlanFilters(f)),usedSafe,counts,recent,{avoidNames,preferIron:true,needsHydrating:hasDense(),avoidDense:hasDense()}); if(!fill)break; lunch.push(fill); usedSafe.add(fill.name);} if(lunch.length<3)return null; if(!isDigestOk([...lunch,...(extraFood?[extraFood]:[])])){const hyd=pickBalanced(getSafeByCat('verdura').filter(f=>f.hydrating),new Set(lunch.map(f=>f.name)),counts,recent,{avoidNames,needsHydrating:true}); if(hyd){const idx=lunch.findIndex(f=>f.dense); if(idx>=0)lunch[idx]=hyd;}}
 return lunch;}
...
function getDailyFruits(b,existing=[]){let pool=safeFruits(); const nf=enrichFood(b.newFood); if(nf&&nf.cat==='fruta'&&b.type==='allergen'&&!isSafe(nf.name))return [nf,nf,nf]; const counts=usageCounts(existing); const recent=recentUseMap(existing,5); const avoidNames=lastBlockFoodNames(existing); const needsHydrating=dedupe([...(b.foods||[]),...(b.newFood?[b.newFood]:[])]).some(f=>f.dense); let res=[]; const used=new Set(); for(let i=0;i<3;i++){ if(i===0&&nf&&nf.cat==='fruta'){res.push(nf); used.add(nf.name); continue;} if(!pool.length)continue; const chosen=pickBalanced(pool,used,counts,recent,{avoidNames,needsHydrating,avoidDense:false}); if(chosen){res.push(chosen); if(pool.length>=3)used.add(chosen.name);} }
 while(res.length<3&&pool.length){const chosen=pickBalanced(pool,new Set(res.map(f=>f.name)),counts,recent,{avoidNames,needsHydrating}); if(chosen)res.push(chosen); else res.push(pool[res.length%pool.length]);}
